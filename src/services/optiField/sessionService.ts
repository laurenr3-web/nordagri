...
export const useSessionService = (currentPosition: FieldPosition | null) => {
  ...
  // Update equipment position when currentPosition changes
  useEffect(() => {
    if (currentPosition && activeSession) {
      updateEquipmentPosition(currentPosition);
      updateSessionPath(activeSession.id, currentPosition);
    }
  }, [currentPosition]);
