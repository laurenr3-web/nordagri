...
const StatusCard: React.FC<StatusCardProps> = ({ equipment }) => {
  ...
  return (
    <Card className={`rounded-2xl border-2 shadow-sm ring-2 ring-offset-1 ${status.ringClass}`}>
      <CardContent className="p-5 space-y-4">
        <div className="flex items-start justify-between gap-3">
          <div className="flex items-center gap-3">
            <div className={`h-10 w-10 rounded-xl flex items-center justify-center ${status.badgeClass} border`}>
            ...
            <div>
              <p className="text-xs font-medium text-muted-foreground uppercase tracking-wide">État actuel</p>
              <h2 className="text-lg font-semibold leading-tight">{status.label}</h2>
            </div>
          </div>
