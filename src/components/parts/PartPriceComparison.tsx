...
const PartPriceComparison: React.FC<PartPriceComparisonProps> = ({ 
  ...
  const loadPriceData = async () => {
    if (!partReference && !partName) {
      toast.error("Informations insuffisantes", {
        description: "Référence ou nom de la pièce requis pour la comparaison de prix"
      });
      return;
    }
    ...
    try {
      ...
      if (data.length === 0) {
        ...
        toast.success(`${data.length} offres trouvées pour cette pièce`);
      }
    } catch (error) {
